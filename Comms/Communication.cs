...
namespace go.Comms
{
  public class Communication
  {
    ...
    private void Login()
    {
      ...
      logon1.checkBoxRememberPassword.Checked = Datas.Communications.rememberPassword;
      Logon logon2 = logon1;
      if (!Datas.Communications.rememberPassword || Datas.Password == "")
      {
        int num = (int) logon2.ShowDialog();
        if (!logon2.logon)
          throw new Exception("Login cancelled by user");
      }
      Datas.Communications.rememberPassword = logon2.checkBoxRememberPassword.Checked;
